...
unsigned char GetScanCode(){
    ...
    if(KBIB->count == 0)
        while (KBIB->count == 0) nop();
    
    if(KBIB->PTail == (KBIB->buf + KB_BUF_SIZE)) KBIB->PTail = KBIB->buf;
    x = *(KBIB->PTail++);
    KBIB->count--;
...
void AnalyzeKeyCode(){
    ...
    if(x == 0xe1){
        for(i = 1; i < PB_CODE_SIZE; i++)
            if(GetScanCode() != PauseBreakCode[i]) break;
    ...
    if(key == 0){
        ...
        if(key) ColorPrintfk(BLUE, BLACK, "%c", key);
    }
...
void KeyboardHandler(struct PtRegs* regs, unsigned long nr, unsigned long arg){
    ...
    if(KBIB->PHead == KBIB->buf + KB_BUF_SIZE) KBIB->PHead = KBIB->buf;
